...
{  
    var status=false;
var apassword=document.f1.apassword.value;  
if(apassword.length<8){  
document.getElementById("apasswordloc").innerHTML=  
...
PreparedStatement getLastInsertId = connection.prepareStatement("SELECT alid FROM applicantlist ORDER BY alid DESC LIMIT 1");
...
--%>
                <form name="f1" onsubmit="return validate();" method="post" action="ApplicServlet">
                    <label for="aname">Username:</label>
                    <input type="text" id="aname" name="aname" class="aname" style="display: block; 
	float: left; 
	width: 240px; 
	padding: 8px; 
	margin-bottom: 20px; 
	...
	font-size: 12px;"/>
                    ...
                    <label for="apassword">Password:</label>
                     <input type="password" id="apassword" name="apassword" class="required input_field" style="float: left;" />
                     <span id="apasswordloc"></span>
                    <div class="clear"></div>
                   ...
                   <input type="hidden" id="uid" name="uid" value="<%=rs.getString("alid")%>" />
